...
class LunarConverter {
    ...
    func getLunarDetails(for date: Date) -> (day: String, month: String, zodiac: String) {
        ...
        // Calculate Zodiac
        // Note: This is a simplified Zodiac calculation based on year index
        // A full implementation would check the exact lunar new year boundary
        ...
        if day == 1 {
            ...
            dayName = "初\(chineseNumbers[day])"
        } else if day < 20 {
            dayName = "十\(chineseNumbers[day - 10])"
        } else if day < 30 {
            dayName = "廿\(chineseNumbers[day - 20])"
        } else {
            dayName = "三\(chineseNumbers[day - 30])"
        }
